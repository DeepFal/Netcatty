...
- 如果用户在快照生成期间开始重连，本次休眠会立即放弃；保护标记一直保留到排队的终端重置完成并真正开始新连接，不会提前释放重连仍要使用的画面；
- 已经轻量隐藏的结束会话会继续升级为完整释放；结束在 Vim、htop 等全屏画面中的会话也可以释放，但仍连接的全屏程序和持有内嵌图片的会话继续保留原有保护；
...
- 休眠状态、无 backend ID 的结束路径、重连竞态、轻量隐藏、全屏画面、内嵌图片、hook 调度、最终释放与流水线结构的专项测试全部通过。
- 在 `ELECTRON_DISABLE_SANDBOX=1` 下，真实 Electron 密集装饰测试通过；本次测得实际关键词高亮装饰 712 个、20,000 个装饰的最慢刷新约 50 ms。
- 生产构建通过。
- 全量测试最终共 8,228 项，其中 8,218 通过、10 跳过、0 失败。此前一次运行里有一个与本次终端改动无关的 AI 网络超时用例偶发失败；该用例随后单独连续运行 5 次通过，之后的完整测试都全部通过。
