...
test("SCP staged uploads skip chmod when existing mode is zero", async (t) => {
  ...
  const backend = {
    ...
    async chmod(remotePath, mode) {
      // Numeric 0 becomes "0", which SCP shell chmod rejects. Skipping is the
      // safe promotion path for unknown/zero modes.
      if (mode === 0 || mode === "0") {
